...
public class Conductor : MonoBehaviour 
{
	...
	[Header("Spawn Points")]
	public PlayerSpawns[] trackSpawnsOffsetX;
    [System.Serializable]
	...
	void Start()
	{
		...
		previousMusicNodes = new MusicNode[partitionLen,len];
        for (int j = 0; j < partitionLen; j++)
        {
            for (int i = 0; i < len; i++)
            {
                trackNextIndices[j,i] = 0;
                queueForTracks[j,i] = new Queue<MusicNode>();
	...
	void Update()
	{
        ...
        for (int j = 0; j < partitionLen; j++)
        {
            for (int i = 0; i < len; i++)
            {
                int nextIndex = trackNextIndices[j,i];
                SongInfo.Track currTrack = partitions[j].tracks[i];
                ...
                if (nextIndex < currTrack.notes.Length && currTrack.notes[nextIndex].note < beatToShow)
                {
                    ...
                    //set z position
                    float layerZ = nextLayerZ[j,i];
                    nextLayerZ[j,i] += LayerOffsetZ;

                    //get a new node
                    MusicNode musicNode = MusicNodePool.instance.GetNode(trackSpawnsOffsetX[j].trackSpawnPosX[i], startLineY, finishLineY, removeLineY, layerZ, currNote.note, currNote.times, trackColors[i]);

                    //enqueue
                    queueForTracks[j,i].Enqueue(musicNode);

                    //update the next index
                    trackNextIndices[j,i]++;
                }
        ...
        for (int j = 0; j < partitionLen; j++)
        {
            for (int i = 0; i < len; i++)
            {
                ...
                if (queueForTracks[j,i].Count == 0) continue;

                MusicNode currNode = queueForTracks[j,i].Peek();

                //multi-times note
                if (currNode.times > 0 && currNode.transform.position.y <= finishLineY + goodOffsetY)
                {
                    //have previous note stuck on the finish line
                    if (previousMusicNodes[j,i] != null)
                    {
                        previousMusicNodes[j,i].MultiTimesFailed();
                    ...
                    previousMusicNodes[j,i] = currNode;
                    queueForTracks[j,i].Dequeue();
                }
